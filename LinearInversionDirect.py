...
class LinearInversionDirectApp(properties.HasProperties):
    ...
    def set_G(self, N=20, M=100, pmin=-0.25, pmax=-15, qmin=0.25, qmax=15):
        ...
        def g(k):
            return np.exp(p_values[k] * self.mesh_prop.cell_centers) * np.cos(
                2*np.pi * q_values[k] * self.mesh_prop.cell_centers
            )
    ...
    def plot_G(
        ...
        ax1.plot(self.mesh_prop.cell_centers, self.G.T)
        if fixed:
    ...
    def set_model(
        ...
        m1_inds = np.logical_and(
            self.mesh_prop.cell_centers > m1_center - dm1 / 2.0,
            self.mesh_prop.cell_centers < m1_center + dm1 / 2.0,
        )
        ...
        m += gaussian(self.mesh_prop.cell_centers, m2_center, sigma_2) * m2
        self.m = m
        return m
    ...
    def plot_model_only(
        ...
        fig = plt.figure()
        ax  = plt.subplot(111)
        ax.plot(self.mesh_prop.cell_centers, m)
        ax.set_ylim([-2.5, 2.5])
        ax.set_title("Model")
        ax.set_xlabel("x")
        ax.set_ylabel("m(x)")
    ...
    def plot_model(
        ...
        if show_model:
            ax1.plot(self.mesh_prop.cell_centers, m)
        ...
        if show_kernel:
            ax2.plot(self.mesh_prop.cell_centers, self.G.T)
    ...
    def run_inversion_direct(
        ...
        data_obj = data.Data(
            survey_obj,
            dobs=self.data_vec, standard_deviation=self.uncertainty
        )
    ...
    def plot_inversion(
        ...
        axes[0].plot(self.mesh_prop.cell_centers, self.m)
        if mode == "Run":
            axes[0].plot(self.mesh_prop.cell_centers, self.model[i_target])
        ...
        if tikhonov == "phi_d & phi_m":
            if mode == "Explore":
                axes[0].plot(self.mesh_prop.cell_centers, self.model[i_beta])
                if data_option == "obs & pred":
